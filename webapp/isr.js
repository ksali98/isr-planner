...
// ----------------------------------------------------
// Segmented Solve Support
// ----------------------------------------------------

/**
 * Build segmented JSON for Agent Solve from cut percentages.
 *
 * @param {number[]} cutPcts - Array of cut percentages (e.g., [0.40, 0.50])
 *                             Each percentage is relative to its segment (0-1).
 *                             Last segment always runs to completion (1.0).
 * @returns {Object} Segmented mission JSON for Agent Solve
 *
 * Example: cutPcts = [0.40, 0.50] produces 3 segments:
 * - Segment 1: 0% → 40% of initial solve
 * - Segment 2: 0% → 50% of segment 2's solve
 * - Segment 3: 0% → 100% (run to completion)
 */
function buildSegmentedMissionJson(cutPcts = []) {
  if (!state.env || !state.routes || Object.keys(state.routes).length === 0) {
    console.warn("buildSegmentedMissionJson: No routes available");
    return null;
  }

  // Build target-to-segment mapping
  const targetSegmentMap = {};

  // No cuts = single segment (backwards compatible)
  if (!cutPcts || cutPcts.length === 0) {
    // All targets belong to segment 1
    (state.env.targets || []).forEach(t => {
      targetSegmentMap[t.id] = 1;
    });
    return {
      segments: [{
        segment_id: 1,
      ...
      targetSegmentMap: targetSegmentMap,
    };
  }

  const segments = [];
  let accumulatedVisitedTargets = [];
  let previousVisitedTargets = [];
  let currentEnv = JSON.parse(JSON.stringify(state.env));
  let currentDroneConfigs = JSON.parse(JSON.stringify(state.droneConfigs));
  let startPositions = null; // First segment starts from airports

  for (let i = 0; i <= cutPcts.length; i++) {
    const isLastSegment = i === cutPcts.length;
    const cutPct = isLastSegment ? 1.0 : cutPcts[i];
    const segmentId = i + 1;

    const segment = {
      segment_id: segmentId,
      cut_pct: cutPct,
      visited_targets: [...accumulatedVisitedTargets],
      env: JSON.parse(JSON.stringify(currentEnv)),
      drone_configs: JSON.parse(JSON.stringify(currentDroneConfigs)),
    };

    // Add start positions for segments after the first
    if (startPositions) {
      segment.start_positions = startPositions;
    }

    // If not the last segment, compute what happens at this cut
    if (!isLastSegment) {
      const cutResult = computeCutAtPercentage(cutPct, currentEnv, accumulatedVisitedTargets);

      segment.end_positions = cutResult.endPositions;

      // Mark newly visited targets as belonging to this segment
      const newlyVisited = cutResult.visitedTargets.filter(
        tid => !previousVisitedTargets.includes(tid)
      );
      newlyVisited.forEach(tid => {
        targetSegmentMap[tid] = segmentId;
      });
      segment.targets_in_segment = newlyVisited;

      // Update for next segment
      previousVisitedTargets = [...cutResult.visitedTargets];
      accumulatedVisitedTargets = [...cutResult.visitedTargets];
      startPositions = cutResult.endPositions;

      // Filter visited targets from env for next segment
      currentEnv = JSON.parse(JSON.stringify(state.env));
      currentEnv.targets = currentEnv.targets.filter(
        t => !accumulatedVisitedTargets.includes(t.id)
      );
    } else {
      // Last segment: remaining targets
      const remainingTargets = (state.env.targets || [])
        .filter(t => !accumulatedVisitedTargets.includes(t.id))
        .map(t => t.id);
      remainingTargets.forEach(tid => {
        targetSegmentMap[tid] = segmentId;
      });
      segment.targets_in_segment = remainingTargets;
    }

    segments.push(segment);
  }

  return { segments, targetSegmentMap };
}

/**
 * Compute what happens at a cut percentage:
 * - Which targets are visited
 * - Drone end positions
 *
 * @param {number} cutPct - Cut percentage (0-1)
 * @param {Object} env - Environment at start of this segment
 * @param {string[]} alreadyVisited - Targets already visited in prior segments
 * @returns {Object} { visitedTargets, endPositions }
 */
function computeCutAtPercentage(cutPct, env, alreadyVisited = []) {
  const visitedTargets = [...alreadyVisited];
  const endPositions = {};
  ...
  Object.entries(state.routes).forEach(([did, routeData]) => {
    ...
    const cutDist = totalDist * cutPct;

    // Find position at cut distance
    let cumDist = 0;
    let cutPoint = traj[0];
    for (let i = 1; i < traj.length; i++) {
      const dx = traj[i][0] - traj[i - 1][0];
      ...
      if (cumDist + segLen >= cutDist) {
        // Interpolate to exact cut point
        const t = (cutDist - cumDist) / segLen;
        cutPoint = [
          traj[i - 1][0] + t * dx,
          traj[i - 1][1] + t * dy
        ];
    ...
    endPositions[did] = cutPoint;

    // Find targets visited up to cut point
    const route = routeData.route || [];
    route.forEach(wp => {
      if (!String(wp).startsWith("T")) return;
      if (visitedTargets.includes(wp)) return;

      const target = env.targets?.find(t => t.id === wp);
      if (!target) return;

      // Find distance to this target on trajectory
      let minDist = Infinity;
      let targetTrajDist = Infinity;
      let cumD = 0;

      for (let i = 0; i < traj.length; i++) {
        const dist = Math.sqrt(
          Math.pow(traj[i][0] - target.x, 2) +
          Math.pow(traj[i][1] - target.y, 2)
        );
        if (dist < minDist) {
          minDist = dist;
          targetTrajDist = cumD;
      ...
      // Target is visited if we passed it (within threshold)
      if (minDist < 20.0 && targetTrajDist <= cutDist) {
        visitedTargets.push(wp);
      }
    });
  });

  return { visitedTargets, endPositions };
}

/**
 * Parse cut percentages from UI input string.
 * Accepts formats like: "40, 50" or "0.4, 0.5" or "40%, 50%"
 *
 * @param {string} inputStr - User input string
 * @returns {number[]} Array of cut percentages (0-1)
 */
function parseCutPercentages(inputStr) {
  ...
  return inputStr
    ...
    .map(s => {
      ...
      // If > 1, assume it's a percentage (e.g., 40 means 40%)
...
/**
 * Get the segmented mission JSON for the current state.
 * Uses the cut percentages from the UI input if available.
 *
 * @returns {Object} Segmented mission JSON
 */
function getSegmentedMissionJson() {
  const cutInput = $("cut-percentages-input");
  const cutPcts = cutInput ? parseCutPercentages(cutInput.value) : [];
  return buildSegmentedMissionJson(cutPcts);
}

/**
 * Update the segment preview UI based on current cut percentages input.
 * Shows segment count badge and preview text.
 */
function updateSegmentPreview() {
  const cutInput = $("cut-percentages-input");
  const badge = $("segment-count-badge");
  const preview = $("segment-preview");

  if (!cutInput) return;

  const cutPcts = parseCutPercentages(cutInput.value);
  const numSegments = cutPcts.length === 0 ? 1 : cutPcts.length + 1;

  // Update badge
  if (badge) {
    badge.textContent = numSegments === 1 ? "1 segment" : `${numSegments} segments`;
    badge.style.background = numSegments === 1 ? "#3b82f6" : "#10b981";
  }

  // Update preview
  if (preview) {
    if (cutPcts.length === 0) {
      preview.textContent = "Single solve (run to completion)";
    } else {
      const segmentDescs = [];
      for (let i = 0; i < cutPcts.length; i++) {
        segmentDescs.push(`S${i + 1}: 0%→${Math.round(cutPcts[i] * 100)}%`);
      }
      segmentDescs.push(`S${cutPcts.length + 1}: 0%→100%`);
      preview.textContent = segmentDescs.join(" | ");
    }
  }
}

/**
 * Attach event listener for cut percentages input.
 */
...
async function runPlanner() {
  ...
  try {
    ...
    // Compute segmented solve data if cut percentages are defined
    const cutInput = $("cut-percentages-input");
    const cutPcts = cutInput ? parseCutPercentages(cutInput.value) : [];
    if (cutPcts.length > 0 && !isCheckpointReplan) {
      // First apply the routes temporarily so buildSegmentedMissionJson can use them
      state.routes = JSON.parse(JSON.stringify(data.routes || {}));

      const segmentedData = buildSegmentedMissionJson(cutPcts);
      if (segmentedData && segmentedData.segments) {
        missionState.draftSolution.segmentedSolve = segmentedData;

        // Store target-to-segment mapping for visualization
        if (segmentedData.targetSegmentMap) {
          state.targetSegmentMap = segmentedData.targetSegmentMap;
        }

        appendDebugLine(`📊 Generated ${segmentedData.segments.length} segments from cut points`);
        segmentedData.segments.forEach((seg, i) => {
          const targetsInSeg = seg.targets_in_segment || [];
          appendDebugLine(`   S${i + 1}: cut@${Math.round(seg.cut_pct * 100)}%, targets=[${targetsInSeg.join(", ")}]`);
        });
      }
    } else {
      // No cut points - clear the segment map
      state.targetSegmentMap = {};
    }
