"""
SAM Navigation module - SINGLE SOURCE OF TRUTH
All SAM avoidance path planning should import from here.
...
class SAMNavigator:
    ...
    def plan_path_with_sam_avoidance(self, start, end, sams, debug=False):
        ...
        Args:
            ...
            sams: List of SAM objects with 'pos' and 'range' keys
            debug: Enable debug output

        Returns:
            Tuple of (path, distance, method_used)
            - path: List of [x, y] waypoints (or None if no valid path)
            - distance: Total path distance (or float('inf') if no valid path)
        ...
        try:
            ...
            if method.startswith("INVALID"):
                if debug:
                    print(f"boundary_navigation returned invalid path: {method}")
            ...
            if path and len(path) >= 2:
                ...
                if debug:
                    print(f"boundary_navigation returned empty path")
    ...
    def _direct_path(self, start, end):
        ...
        return path, float(distance), "Direct path"
